...
impl MetaLanguageExecutor {
    ...
    pub fn process_document(&mut self, content: &str) -> Result<(), ExecutorError> {
        ...
        // PHASE 2: Process ALL blocks for variable references in multiple passes
        ...
        // Do multiple passes (up to 3) to handle nested references
        for pass in 0..3 {
            ...
            if !any_replaced {
                println!(
                    "DEBUG: No more variable references to resolve after pass {}",
                    pass + 1
                );
                break;
            }
